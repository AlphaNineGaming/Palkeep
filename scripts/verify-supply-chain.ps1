...
$trackedNative = & git -C $root ls-files single-player 2>$null |
  Where-Object { $_ -match "\.(dll|exe|pyd|so|dylib|zip|cat)$" }
if ($trackedNative) {
  throw "Native files are tracked in single-player:`n$($trackedNative -join "`n")"
}
...
Write-Host "Supply-chain verification passed: no tracked single-player binaries, official Python signature valid, source-built ooz verified."
